...
namespace Native.App.Export
{
	...
	{	
		...
		{	
			return "9,cn.mikualpha.kancolle.time";	
		}	
		...
		{	
			...
			Type appDataType = typeof (AppData);	
			// 注册一个 CQApi 实例	
			AppInfo appInfo = new AppInfo ("cn.mikualpha.kancolle.time", 1, 9, "KanColle语音报时", "1.1.3", 6, "初音未来Alpha", "KanColle语音报时", authCode);	
			appDataType.GetRuntimeProperty ("CQApi").GetSetMethod (true).Invoke (null, new object[] { new CQApi (appInfo) });	
			AppData.UnityContainer.RegisterInstance<CQApi> ("cn.mikualpha.kancolle.time", AppData.CQApi);	
			// 向容器注册一个 CQLog 实例	
			appDataType.GetRuntimeProperty ("CQLog").GetSetMethod (true).Invoke (null, new object[] { new CQLog (authCode) });	
			AppData.UnityContainer.RegisterInstance<CQLog> ("cn.mikualpha.kancolle.time", AppData.CQLog);	
